...
class KPI_Calculator(object):
        ...
        for source in self.sources_tdis:
            ...
            for signal in self.case.kpi_json[source]:
                ...
                self.tdis_dict[signal[:-1]+'dTlower_y'] += \
                    np.trapz(dT_lower,self._get_data_from_last_index('time',self.i_last_tdis))/3600.
                self.tdis_dict[signal[:-1]+'dTupper_y'] += \
                    np.trapz(dT_upper,self._get_data_from_last_index('time',self.i_last_tdis))/3600.
        ...
        for source in self.sources_idis:
            ...
            for signal in self.case.kpi_json[source]:
                ...
                self.idis_dict[signal[:-1]+'dIupper_y'] += \
                    np.trapz(dI_upper, self._get_data_from_last_index('time',self.i_last_idis))/3600.
        ...
        for source in self.sources_cost:
            ...
            for signal in self.case.kpi_json[source]:
                ...
                self.cost_dict[signal] += \
                    np.trapz(np.multiply(source_price_data,pow_data),
            ...
            if 'Power' in source:
                ...
                for signal in self.case.kpi_json[source]:
                    ...
                    self.emis_dict[signal] += \
                        np.trapz(np.multiply(source_emissions_data,pow_data),
    ...
    def get_actuator_travel(self):
        ...
        for source in self.sources_ltra:
            for signal in self.ltra_source_key_mapping[source]:
                ...
                self.ltra_tot = self.ltra_tot/len(self.ltra_dict) #Divide by the number of the actuators
    ...
    def _displacement(self, x, y, a, b):
        ...
        value = np.trapz(integrand, x[bounds])
        return float(value)
