...
def parse_instances(model_path, file_name):
    ...
    for var in allvars:
        ...
        if label is not 'kpi':
            ...
            # Add CAT qualifier for ControlActuatorTravel
            if signal_type == 'ControlActuatorTravel':
                __cat_val = int(fmu.get(instance+'.CAT')[0])
                __cat_map = {
                    1: 'None', 2: 'Damper', 3: 'Valve', 4: 'Fan', 5: 'Pump', 6: 'HVACEquipment', 7: 'Others'
                }
                signal_type = 'ControlActuator[{0}]'.format(__cat_map.get(__cat_val, str(__cat_val)))
