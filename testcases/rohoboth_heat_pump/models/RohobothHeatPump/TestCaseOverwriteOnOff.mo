...
model TestCaseOverwriteOnOff
  ...
  Buildings.Utilities.IO.SignalExchange.Overwrite oveFan(u(
      unit="1",
      min=0,
      max=1), description="Real signal to control the fan on off signal (any value less than 0.1 - off, any value larger or equal than 0.1 - on")
    "Block for overwriting fan on off signal" annotation (Placement(
        transformation(
        extent={{4,4},{-4,-4}},
        rotation=180,
        origin={-36,-76})));
  Buildings.Utilities.IO.SignalExchange.Overwrite oveCooCoi(u(
      unit="1",
      min=0,
      max=1), description="Real signal to control the DX cooling coil on off signal (any value less than 0.1 - off, any value larger or equal than 0.1 - on")
    "Block for overwriting DX cooling coil on off signal" annotation (Placement(
        transformation(
        extent={{4,4},{-4,-4}},
        rotation=180,
        origin={-36,-52})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea1
    "Convert fan enable signal to real value"
    annotation (Placement(transformation(extent={{-52,-56},{-44,-48}})));
  Modelica.Blocks.Math.RealToBoolean realToBoolean1(threshold=0.1)
    annotation (Placement(transformation(extent={{-28,-56},{-20,-48}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea2
    "Convert fan enable signal to real value"
    annotation (Placement(transformation(extent={{-52,-68},{-44,-60}})));
  Buildings.Utilities.IO.SignalExchange.Overwrite oveHeaCoi(u(
      unit="1",
      min=0,
      max=1), description="Real signal to control the DX heating coil on off signal (any value less than 0.1 - off, any value larger or equal than 0.1 - on")
    "Block for overwriting DX heating coil on off signal" annotation (Placement(
        transformation(
        extent={{4,4},{-4,-4}},
        rotation=180,
        origin={-36,-64})));
  Modelica.Blocks.Math.RealToBoolean realToBoolean2(threshold=0.1)
    annotation (Placement(transformation(extent={{-28,-68},{-20,-60}})));
equation
  connect(fanOpeMod.y, modCon.fanOpeMod) annotation (Line(points={{-108,-80},{-100,
          -80},{-100,-73.4},{-82,-73.4}}, color={255,0,255}));
  connect(pthp.yFan_actual, fanProOn.u) annotation (Line(points={{27,10},{34,10}},
                            color={0,0,127}));
  connect(pthp.port_Air_a2, zon.ports[1])
    annotation (Line(points={{26,-2},{76,-2},{76,30.9}},
                                                       color={0,127,255}));
  connect(pthp.port_Air_b2, zon.ports[2])
    annotation (Line(points={{26,-10},{80,-10},{80,30.9}},
                                                         color={0,127,255}));
  ...
  connect(pthp.TAirSup, modCon.TSup) annotation (Line(points={{27,4},{30,4},{30,
  ...
  connect(building.weaBus,pthp. weaBus) annotation (Line(
      points={{10,128},{14,128},{14,66},{-10.2,66},{-10.2,-0.4}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(TSupMea.u, pthp.TAirSup) annotation (Line(points={{78,-60},{34,-60},{34,
          4},{27,4}},    color={0,0,127}));
  ...
  connect(realToBoolean.y, booleanToReal.u) annotation (Line(points={{-41,120},{
          -34,120},{-34,106},{-134,106},{-134,90},{-122,90}},  color={255,0,255}));
  connect(booleanToReal.y, OccSchMea.u)
    annotation (Line(points={{-99,90},{-88,90}}, color={0,0,127}));
  connect(realToBoolean.y, modCon.uAva) annotation (Line(points={{-41,120},{-32,
          120},{-32,-44},{-104,-44},{-104,-70},{-82,-70}}, color={255,0,255}));
  connect(pthp.yFan_actual, yFanMea.u) annotation (Line(points={{27,10},{32,10},
  ...
  connect(uSupHea.y, pthp.uSupHea)
    annotation (Line(points={{-40,-8},{-15,-8}}, color={0,0,127}));
  connect(modCon.yFan, booToRea.u)
    annotation (Line(points={{-58,-76},{-52.8,-76}}, color={255,0,255}));
  connect(booToRea.y, oveFan.u)
    annotation (Line(points={{-43.2,-76},{-40.8,-76}}, color={0,0,127}));
  connect(oveFan.y, pthp.uFan) annotation (Line(points={{-31.6,-76},{-18,-76},{-18,
          8},{-15,8}}, color={0,0,127}));
  connect(booToRea1.y, oveCooCoi.u)
    annotation (Line(points={{-43.2,-52},{-40.8,-52}}, color={0,0,127}));
  connect(modCon.yCooEna, booToRea1.u)
    annotation (Line(points={{-58,-52},{-52.8,-52}}, color={255,0,255}));
  connect(oveCooCoi.y, realToBoolean1.u)
    annotation (Line(points={{-31.6,-52},{-28.8,-52}}, color={0,0,127}));
  connect(realToBoolean1.y, pthp.uCooEna) annotation (Line(points={{-19.6,-52},{
          -18,-52},{-18,-20},{-15,-20}}, color={255,0,255}));
  connect(oveHeaCoi.y, realToBoolean2.u)
    annotation (Line(points={{-31.6,-64},{-28.8,-64}}, color={0,0,127}));
  connect(booToRea2.y, oveHeaCoi.u)
    annotation (Line(points={{-43.2,-64},{-40.8,-64}}, color={0,0,127}));
  connect(modCon.yHeaEna, booToRea2.u) annotation (Line(points={{-58,-56},{-56,-56},
          {-56,-64},{-52.8,-64}}, color={255,0,255}));
  connect(realToBoolean2.y, pthp.uHeaEna) annotation (Line(points={{-19.6,-64},{
          -18,-64},{-18,-24},{-15,-24}}, color={255,0,255}));
